...
const ThreeDemo: React.FC = () => {
  ...
  const createTags = (dom: HTMLElement, info: any) => {
    ...
    TAG.position.set(targetPosition?.x, targetPosition?.y + 0.5, targetPosition?.z);
    TAG.name = info.name;
    let hasTag = false;
    viewer?.scene?.traverse((child) => {
      if (child instanceof CSS2DObject && child.name === info.name) {
  ...
  const initModel = () => {
    modelLoader.loadModelToScene("/models/datacenter.glb", (baseModel) => {
      ...
      const rackList: Object3DExtends[] = [];
      const allList: Object3DExtends[] = [];
      const chairList: Object3DExtends[] = [];
      model.traverse((item) => {
        if (checkIsRack(item)) {
          rackList.push(item);
        }
  ...
  return (
    <div className={styles.wrapper}>
      <div id={PAGE_ID} style={{ width: 1000, height: 1000, border: "1px solid red" }}></div>

      {deviceListData?.map((item, index) => {
        return (
          <Popover
            key={item?.name}
            ref={(el) => (tagRefs.current[index] = { dom: el, ...item } as Object3DExtends)}
